...
export default function AddSelectionMenu() {
  ...
  const setPitchDeck = useStore((state) => state.setPitchDeck);
  const setConversationMessageId = useStore(
    (state) => state.setConversationMessageId
  );
  ...
  const onSelectionAnnotate = async () => {
    ...
    if (range && selection && !selection.isCollapsed) {
      ...
      const { pitchDeck: updatedPitchDeck, conversationMessage } =
        await utils.client.pitchDeck.addMessage.mutate({
          id: pitchDeck?.id!,
          body: "",
          details,
        });
